...
let cachedExperience: LoadedExperience | null = null;
...
interface SeedFailures {
  failure_families: Array<{
    id: string;
    name: string;
    symptoms: string[];
    match?: Array<{ type: string; field: string; pattern: string }>;
    recovery: string[];
    recovery_id?: string;
    notes?: string;
  }>;
...
interface SeedRecoveries {
  recovery_sequences: Array<{
    id: string;
    name: string;
    trigger: string;
    steps: string[];
...
interface SeedQuirks {
  platform_quirks: Record<string, Array<{
    id: string;
    quirk: string;
    workaround?: string;
    command?: string;
...
function loadSeedFamilies(): FailureFamily[] {
  const seedDir = getSeedDir();
  const data = loadYaml<SeedFailures>(join(seedDir, 'common-failures.yaml'));
  if (!data?.failure_families) return [];

  return data.failure_families.map(f => ({
    id: f.id,
    name: f.name,
    symptoms: f.symptoms,
    match: f.match as FailureFamily['match'],
    recovery: f.recovery,
...
function parseRecoveryStep(step: string): RecoveryStep {
  ...
  if (waitMatch) return { kind: 'wait', ms: parseInt(waitMatch[1], 10) * 1000 };

  if (/^\w+$/.test(step.trim())) return { kind: 'tool', tool: step.trim() };

  return { kind: 'tool', tool: step };
}

function loadSeedRecoveries(): RecoverySequence[] {
  const seedDir = getSeedDir();
  const data = loadYaml<SeedRecoveries>(join(seedDir, 'recovery-playbook.yaml'));
  if (!data?.recovery_sequences) return [];
...
function loadSeedHeuristics(): ExperienceHeuristic[] {
  const seedDir = getSeedDir();
  const heuristics: ExperienceHeuristic[] = [];

  const quirks = loadYaml<SeedQuirks>(join(seedDir, 'platform-quirks.yaml'));
    ...
    for (const [platform, items] of Object.entries(quirks.platform_quirks)) {
      for (const item of items) {
        heuristics.push({
          ...
          env_filter: { platform: platform as 'ios' | 'android' },
          confidence: 90,
        });
      }
    }
  ...
  const gotchas = loadYaml<SeedGotchas>(join(seedDir, 'expo-gotchas.yaml'));
...
function parseExperienceMd(path: string, source: HeuristicSource): ExperienceHeuristic[] {
  try {
    ...
    while ((match = sectionRe.exec(content)) !== null) {
      ...
      heuristics.push({
        id: `${prefix}-${num}`,
        source,
        type: type as ExperienceHeuristic['type'],
        summary: title,
        confidence,
      });
    }
...
function matchesEnvironment(
  filter: Partial<EnvironmentFingerprint> | undefined,
  ...
  if (filter.platform) {
    checks++;
    if (env.platform === filter.platform) matches++;
    else if (env.platform === null) { matches += 0.5; score *= 0.8; }
  }

  if (filter.engine) {
    checks++;
    if (env.engine === filter.engine) matches++;
    else if (env.engine === null) { matches += 0.5; score *= 0.8; }
  }

  if (filter.architecture) {
    checks++;
    if (env.architecture === filter.architecture) matches++;
    else if (env.architecture === null) { matches += 0.5; score *= 0.8; }
  }
  ...
  if (filter.rn_version && env.rn_version) {
    ...
    if (fMaj === eMaj && fMin === eMin) matches++;
    else if (fMaj === eMaj) { matches += 0.7; score *= 0.9; }
  }
  ...
  if (filter.expo_sdk && env.expo_sdk) {
    ...
    if (fMaj === eMaj) matches++;
  }

  if (checks === 0) return score;
  return score * (matches / checks);
}

// --- Public API ---

export function loadExperience(forceReload = false): LoadedExperience {
  if (cachedExperience && !forceReload) return cachedExperience;
  ...
  try {
    let dir = process.cwd();
    for (let i = 0; i < 10; i++) {
      if (existsSync(join(dir, 'package.json'))) { projectRoot = dir; break; }
  ...
  const byId = new Map<string, ExperienceHeuristic>();
  for (const h of seedHeuristics) byId.set(h.id, h);
  for (const h of projectHeuristics) byId.set(h.id, h);
  for (const h of userHeuristics) byId.set(h.id, h);
  ...
  for (const h of filtered) {
    ...
    if (tokenEstimate + tokens > 2000) break;
    heuristics.push(h);
...
export function getFailureFamilies(): FailureFamily[] {
  return loadExperience().families;
}

export function getRecoverySequence(id: string): RecoverySequence | undefined {
