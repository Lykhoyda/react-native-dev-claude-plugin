  ...
  return withConnection(getClient, async (_args: { full: boolean }, client) => {
    ...
    try {
      const result = await client.evaluate(
        '(function() { var ds = typeof __turboModuleProxy === "function" ? __turboModuleProxy("DevSettings") : null; if (!ds) try { ds = require("react-native").DevSettings; } catch(e) {} if (ds && ds.reload) ds.reload(); else throw new Error("DevSettings not available"); })()'
      );
      if (result.error) {
        ...
        if (check.value === 'undefined') {
          ...
          if (ok) {
            return okResult(
              ...
              { meta: { note: 'Bridgeless reload — WS stayed open, helpers re-injected.' } },
            );
          }
        }
      }
    ...
    if (!client.isConnected) {
      return okResult(
        ...
        { meta: { warning: 'Reload triggered but reconnection timed out. Call cdp_status to check state.' } },
      );
    }
