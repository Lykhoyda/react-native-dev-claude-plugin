...
export function createNetworkBodyHandler(getClient) {
    return withConnection(getClient, async (args, client) => {
        ...
        if (client.networkMode === 'cdp') {
            try {
                ...
                if (msg.includes('No resource with given identifier') || msg.includes('No data found')) {
                    return failResult(`Response body not available for ${args.requestId}. The response may not have finished loading.`, { hint: 'Check that the request has bodyAvailable: true in cdp_network_log output.' });
                }
        ...
        return failResult('Network monitoring is not active. Neither CDP Network domain nor hook fallback is enabled.', { hint: 'Call cdp_status to check network capabilities.' });
    });
}
