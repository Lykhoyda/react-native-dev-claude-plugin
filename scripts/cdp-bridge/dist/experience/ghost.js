...
function isOnCooldown(familyId) {
 ...
 */
export async function attemptGhostRecovery(ghost) {
    // Guard: no recursive ghost
    if (ghost.context.depth > 0 || ghost.context.disable_ghost)
        return null;
    // Guard: check RN_AGENT_NO_GHOST env (for plugin debugging)
    if (process.env.RN_AGENT_NO_GHOST === '1')
        return null;
    ...
    if (!classification.ghost_eligible)
        return null;
    if (!GHOST_WHITELIST.has(classification.family_id))
        return null;
    // Guard: confidence threshold (per Gemini review)
    if (classification.confidence < 0.5)
        return null;
    // Guard: cooldown
    if (isOnCooldown(classification.family_id))
        return null;
    // Mark cooldown
    ...
    try {
        ...
        const recovered = await Promise.race([
            executeRecovery(classification, ghost).catch(() => false),
            new Promise((resolve) => setTimeout(() => resolve(false), GHOST_TIMEOUT_MS)),
        ]);
        ...
        const result = {
            ...
            steps_executed: didRecover && typeof recovered === 'object' ? recovered.steps : 0,
            latency_ms: latency,
            note: didRecover
 ...
 */
async function executeRecovery(classification, ghost) {
 ...
 */
export function appendGhostNote(result, ghostResult) {
    if (!result || typeof result !== 'object')
        return result;
    const envelope = result;
